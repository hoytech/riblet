...
CodedSymbol := <symbolLen (Uint32LE)> || <countDelta (VarintZ)> || <hash (Byte[16])> || <value (Bytes)>
...
struct FileReader {
    ...
    std::optional<CodedSymbol> readSymbol() {
        ...
        auto hash = getBytes(v, 16);

        currCount += countDelta;

        CodedSymbol sym(v, Hash::raw(hash), currCount);
